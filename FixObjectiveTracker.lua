-----------------------------------------------------------------------------------------------
-- Client Lua Script for FixObjectiveTracker
...
local FixObjectiveTracker = {}

function FixObjectiveTracker:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
...
function FixObjectiveTracker:Init()
    Apollo.RegisterAddon(self)
end

function FixObjectiveTracker:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("FixObjectiveTracker.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function FixObjectiveTracker:OnSave(eType)
...
function FixObjectiveTracker:OnRestore(eType, tSavedData)
	if eType ~= GameLib.CodeEnumAddonSaveLevel.Character then
		return
	end
...
function FixObjectiveTracker:OnDocumentReady()
    ...
    self.wndMain = Apollo.LoadForm(self.xmlDoc, "FixObjectiveTrackerForm", "FixedHudStratumLow", self)
...
function FixObjectiveTracker:OnRedrawTimer()
...
function FixObjectiveTracker:OnRequestParent()
	Event_FireGenericEvent("ObjectiveTrackerLoaded", self.wndObjectiveTrackerScroll)
end

function FixObjectiveTracker:OnWindowManagementReady()
...
function FixObjectiveTracker:OnWindowManagementUpdate(tSettings)
...
function FixObjectiveTracker:HelperConstructDataBtnAdded(wndButton)
...
function FixObjectiveTracker:OnBackgroundMouse(wndHandler, wndControl)
...
function FixObjectiveTracker:OnOptionsUpdated()
...
function FixObjectiveTracker:OnAddonNew(tAddonSettings)
...
function FixObjectiveTracker:OnAddonRemoved(tAddonSettings)
...
function FixObjectiveTracker:OnAddonUpdate(tAddonSettings)
...
function FixObjectiveTracker:OnResizeAll()
...
function FixObjectiveTracker:OnListBtnClick(wndHandler, wndControl, eMouseButton)
...
-- Right Click
-----------------------------------------------------------------------------------------------
function FixObjectiveTracker:CloseContextMenu() -- From a variety of source
...
function FixObjectiveTracker:DrawContextMenu()
...
function FixObjectiveTracker:OnToggleQuestTrackerByDistance(wndHandler, wndControl)
...
function FixObjectiveTracker:OnToggleQuestTrackerAlignTop(wndHandler, wndControl)
	g_InterfaceOptions.Carbine.bQuestTrackerAlignBottom = not wndHandler:IsChecked()
	Event_FireGenericEvent("OptionsUpdated_QuestTracker")
end

function FixObjectiveTracker:OnToggleQuestTrackerAlignBottom(wndHandler, wndControl)
	g_InterfaceOptions.Carbine.bQuestTrackerAlignBottom = wndHandler:IsChecked()
	Event_FireGenericEvent("OptionsUpdated_QuestTracker")
end

function FixObjectiveTracker:OnToggleQuestShowButtons(wndHandler, wndControl)
	self.bShowButtonsOnMouseOver = wndHandler:IsChecked()
	
	self.wndButtonContainer:Show(not self.bShowButtonsOnMouseOver)
end

function FixObjectiveTracker:OnOpacitySliderBarChanged(wndHandler, wndControl, fValue, fOldValue)
...
function FixObjectiveTracker:OnOpacityMouseSliderBarChanged(wndHandler, wndControl, fValue, fOldValue)
...
local FixObjectiveTrackerInst = FixObjectiveTracker:new()
FixObjectiveTrackerInst:Init()

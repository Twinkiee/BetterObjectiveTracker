...
-----------------------------------------------------------------------------------------------
--MovableObjectiveTracker Events
-----------------------------------------------------------------------------------------------
...
function Navpoints:HelperUpdateObjectiveTracker()
	local tData = {
		["strAddon"]	= kstrObjectiveType,
	}
...
function Navpoints:OnClearNavpointBtn()
	...
	self:HelperUpdateObjectiveTracker()
end
...
function Navpoints:HelperResize(bExpand)
	...
	local nNewBottom = self.nOriginalBottom
	if bExpand then
		local nHeight = self.wndListItem:GetHeight()
		nNewBottom = nTop + nHeight + nHeight / 2
	end
	self.wndMain:SetAnchorOffsets(nLeft, nTop, nRight, nNewBottom)
	self:HelperUpdateObjectiveTracker()
end
